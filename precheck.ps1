...
<style>
    ...
    th {
        background: #717074;
        background: linear-gradient(#464547, #717074);
        color: #fff;
        font-size: 11px;
...
$dnsHtml = Show-DnsHealth -json .\SoS-JSON-HealthCheck-Samples\dns-health-results.json -html
...
$report = ConvertTo-HTML -Body "$reportTitle $backupUserHtml $dnsHtml $systemPasswordHtml $datastoreTitle $allStorageCapacityHtml $coreDumpTitle $allEsxiCoreDumpHtml " -Title "SDDC Manager Health Check Report" -Head $reportFormat -PostContent "<p>Creation Date: $(Get-Date)<p>"
